(() => {
  ...
  let continuousFillActive = false;
  ...
  // All ordinary controls are routed through this small adapter contract.  The
  // framework adapters intentionally share the same event/value machinery,
  // while their detect() functions make later site-specific extensions local.
  const COMPONENT_ADAPTERS = [
    {
      id: 'antd',
      detect: el => /ant-(select|picker|cascader|auto-complete)/.test(String(el.className || '') + String(el.parentElement?.className || '')),
      describe: el => ({label: labelFor(el), options: []}),
      fill: async (item, value) => fillFrameworkControl(item, value),
      read: item => readControlValue(item.element),
      validate: (item, value) => !!readControlValue(item.element) || value === '',
    },
    {
      id: 'element',
      detect: el => /el-(select|date|cascader|autocomplete)/.test(String(el.className || '') + String(el.parentElement?.className || '')),
      describe: el => ({label: labelFor(el), options: []}),
      fill: async (item, value) => fillFrameworkControl(item, value),
      read: item => readControlValue(item.element),
      validate: (item, value) => !!readControlValue(item.element) || value === '',
    ...
    {
      id: 'arco',
      detect: el => /arco-(select|picker|cascader|auto-complete)/.test(String(el.className || '') + String(el.parentElement?.className || '')),
      describe: el => ({label: labelFor(el), options: []}),
      fill: async (item, value) => fillFrameworkControl(item, value),
      read: item => readControlValue(item.element),
      validate: (item, value) => !!readControlValue(item.element) || value === '',
    },
    {
      ...
      detect: () => true,
      describe: el => ({label: labelFor(el), options: []}),
      fill: async (item, value) => fillNativeControl(item, value),
      read: item => readControlValue(item.element),
      validate: (item, value) => item.elements ? item.elements.some(el => el.checked) : (!!readControlValue(item.element) || value === ''),
    },
  ];

  function adapterFor(el) { return COMPONENT_ADAPTERS.find(adapter => adapter.detect(el)) || COMPONENT_ADAPTERS.at(-1); }
  ...
  function scanFields(root = document, fields = null) {
    if (!fields) {
      registry = new Map();
      fields = [];
    }
    const elements = [...root.querySelectorAll('input, textarea, select, [role="combobox"]')]
      .filter(isVisible)
      .filter(el => !(el.getAttribute('role') === 'combobox' && el.querySelector('input')));
    ...
    for (const el of elements) {
      ...
      const meta = {
        ...
        name: el.name || '',
        type: el.tagName === 'SELECT' ? 'select' : (el.getAttribute('role') === 'combobox' ? 'combobox' : inputType),
        options,
        context: contextFor(el),
      };
      meta.fingerprint = fingerprint(meta);
  ...
  async function fillNativeControl(item, value) {
    if (item.kind === 'radio' || item.kind === 'checkbox') return chooseGroup(item.elements, value);
    if (item.kind === 'select') return chooseSelect(item.element, value);
    if (dateLike(item.element, item.meta.label)) return setDateControl(item.element, value, item.meta.label);
    setNativeValue(item.element, value);
    return true;
  }

  async function fillFrameworkControl(item, value) {
    if (dateLike(item.element, item.meta.label)) return setDateControl(item.element, value, item.meta.label);
    return chooseCustomSelectDirect(item.element, value);
  }
  ...
  async function fillOne(match) {
    ...
    try {
      const adapter = adapterFor(item.element || item.elements?.[0]);
      const filled = await adapter.fill(item, match.fill_value);
      return filled && adapter.validate(item, match.fill_value);
  ...
  async function api(path, options = {}) {
    const response = await chrome.runtime.sendMessage({type: 'JOB_AUTOFILL_API', path, options});
    if (!response?.ok) throw new Error(response?.error || '本机服务未响应');
  ...
  function setManualTarget(node) {
    ...
    chrome.runtime.sendMessage({type: 'JOB_AUTOFILL_TARGET_CHANGED'}).catch(() => {});
  }
  ...
  function stableFormSignature(fields) {
    const text = [...new Set(fields.map(field => field.fingerprint).filter(Boolean))].sort().join('\n');
    let hash = 2166136261;
  ...
  async function run() {
    ...
    const formSignature = stableFormSignature(fields);
    const data = await api('/api/match', {method: 'POST', body: JSON.stringify({
      page_url: location.href, form_signature: formSignature, fields,
    })});
    let filled = 0;
    const pending = [];
    for (const match of data.matches || []) {
      if (match.needs_confirmation) {
        const field = fields.find(item => item.id === match.id);
        if (field) pending.push({field, match});
      } else if (await fillOne(match)) filled += 1;
    }

    if (pending.length) {
      await chrome.runtime.sendMessage({type: 'JOB_AUTOFILL_REVIEW', review: {
        pageUrl: location.href, formSignature, pending,
      }});
    }

    let profile = {};
    try { profile = (await api('/api/profile')).profile || {}; } catch (_) {}
    ...
    return {...data.stats, filled, pending: pending.length, formSignature, repeatable: {internships, awards, family}};
  }

  function startContinuousSession() {
    if (continuousFillActive) return;
    continuousFillActive = true;
    const observer = new MutationObserver(() => {
      clearTimeout(mutationTimer);
      mutationTimer = setTimeout(async () => {
        if (!continuousFillActive) return;
        const signature = stableFormSignature(scanAllFields());
        if (handledSignatures.has(signature)) return;
        try {
          const stats = await run();
          handledSignatures.add(stats.formSignature);
        } catch (_) {}
      }, 300);
    });
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (message?.type === 'JOB_AUTOFILL_RUN' || message?.type === 'JOB_AUTOFILL_APPLY_CONFIRMED') {
      if (message.type === 'JOB_AUTOFILL_APPLY_CONFIRMED') handledSignatures.clear();
    ...
    if (message?.type === 'JOB_AUTOFILL_MANUAL_VALUE') {
      fillManualTarget(message.value)
        .then(result => sendResponse({ok: result.ok, error: result.reason || ''}))
        .catch(err => sendResponse({ok: false, error: err.message || String(err)}));
      return true;
    }
